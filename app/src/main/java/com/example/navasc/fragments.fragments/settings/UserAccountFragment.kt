...
//import com.example.navasc.databinding.FragmentUserAccountBinding
//import com.example.navasc.util.Resource
//import com.example.navasc.viewmodel.UserAccountViewModel
...
class UserAccountFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launchWhenStarted {
            viewModel.user.collectLatest { userResource ->
                when (userResource) {
                    ...
                    is Resource.Success -> {
                        hideUserLoading()
                        showUserInformation(userResource.data!!)
                    }
                    is Resource.Error -> Toast.makeText(requireContext(), userResource.message, Toast.LENGTH_SHORT).show()
                    else -> Unit
                }
            }
        }
        ...
        lifecycleScope.launchWhenStarted {
            viewModel.updateInfo.collectLatest { updateInfoResource ->
                when (updateInfoResource) {
                    is Resource.Loading -> binding.buttonSave.startAnimation()
                    is Resource.Success -> {
                        binding.buttonSave.revertAnimation()
                        findNavController().navigateUp()
                    }
                    is Resource.Error -> Toast.makeText(requireContext(), updateInfoResource.message, Toast.LENGTH_SHORT).show()
                    else -> Unit
                }
            }
        }
        ...
        binding.buttonSave.setOnClickListener {
            binding.apply {
                ...
                val email = edEmail.text.toString().trim()
                val user = User(firstName, lastName, email)
                imageUri?.let { viewModel.updateUser(user, it) }
            }
        }
